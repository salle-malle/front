...
export default function ScrapPage() {
  ...
  // 그룹 상세 정보 관련 상태
  const [groupDetailSnapshots, setGroupDetailSnapshots] = useState<
  ...
  // 날짜별 분류 확인
  const stockDates = selectedStockSnapshots.map(
    (s) => s.snapshotCreatedAt.split("T")[0]
  );
    ...
    try {
      ...
      if (response?.status && response.data) {
        ...
        const sortedSnapshots = snapshots.sort(
          (a, b) =>
            new Date(a.snapshotCreatedAt).getTime() -
            new Date(b.snapshotCreatedAt).getTime()
        );
  ...
  const handleStockClick = async (stockCode: string) => {
    ...
    if (snapshots && snapshots.length > 0) {
      ...
      const snapshotsByDate: {
        [date: string]: MemberStockSnapshotDetailResponseDto[];
      } = {};
      ...
      if (dates.length > 0) {
        ...
        const targetIndex = snapshots.findIndex(
          (s) =>
            s.snapshotId ===
            targetDateSnapshots[targetDateSnapshots.length - 1].snapshotId
        );
  ...
  const handleDateChange = async (date: string) => {
    ...
    console.log(
      "Current groupDetailSnapshotsByDate keys:",
      Object.keys(groupDetailSnapshotsByDate)
    );
    console.log(
      "Available dates in groupDetailSnapshotsByDate:",
      Object.keys(groupDetailSnapshotsByDate).sort()
    );
  ...
  useEffect(() => {
    ...
    if (groupDetailSnapshots.length > 0 && selectedGroupId !== null) {
      ...
      if (dates.length > 0) {
        ...
        const isTodayOrInvalidDate =
          selectedDate === today || !dates.includes(selectedDate);

        if (isTodayOrInvalidDate && mostRecentDate !== selectedDate) {
          console.log(
            "Moving to most recent group detail date:",
            mostRecentDate
          );
          setSelectedDate(mostRecentDate);
    ...
    if (selectedStockSnapshots.length > 0) {
      const newIndex =
        direction === "left"
          ...
          : Math.min(
              selectedStockSnapshots.length - 1,
              currentStockSnapshotIndex + 1
            );
  ...
  const handleStockSnapshotIndexChange = (newIndex: number) => {
    ...
    if (
      ...
      newIndex < selectedStockSnapshots.length
    ) {
  ...
  const handleGroupDetailDelete = async (snapshotId: number) => {
    try {
      ...
      const snapshot = groupDetailSnapshots.find(
      ...
      if (response.status) {
        ...
        setGroupDetailSnapshots((prev) => {
          const filteredSnapshots = prev.filter(
          ...
          setCurrentGroupDetailIndex((currentIndex) => {
            ...
            if (filteredSnapshots.length === 0) {
              ...
              if (currentIndex >= filteredSnapshots.length) {
                ...
                console.log(
                  "Current card deleted, moving to last card, newIndex:",
                  newIndex
                );
                return newIndex;
              } else {
                console.log(
                  "Current card deleted, staying at same position, newIndex:",
                  currentIndex
                );
              ...
              console.log(
                "Card before current deleted, adjusting index, newIndex:",
                newIndex
              );
              return newIndex;
            } else {
              ...
              // 현재 인덱스 유지
              console.log(
                "Card after current deleted, keeping same index, newIndex:",
                currentIndex
              );
              return currentIndex;
            }
      ...
      <div className="relative flex-1">
          ...
          unifiedStocks?.stocks ? (
            ...
            <>
              {isLoading ? (
                ...
                <ScrapCardViewer
                  cards={currentGroupDetailSnapshots.map(
                    convertGroupDetailToSnapshotCard
                  )}
                ...
                (() => {
                  ...
                  console.log(
                    "filteredStockSnapshots 개수:",
                    filteredStockSnapshots.length
                  );
                  console.log(
                    "filteredStockSnapshots 데이터:",
                    filteredStockSnapshots
                  );
                  console.log(
                    "currentStockSnapshotIndex:",
                    currentStockSnapshotIndex
                  );
                  console.log("selectedStockCode:", selectedStockCode);
                  console.log("activeView:", activeView);

                  const convertedCards = filteredStockSnapshots.map(
                    convertToSnapshotCard
                  );
        ...
        <div
          ...
          <TrebleSelector
            ...
            snapshotsForDate={
              groupDetailSnapshots.length > 0
                ? currentGroupDetailSnapshots.map(
                    convertGroupDetailToSnapshotCard
                  )
                : selectedStockSnapshots.length > 0
            ...
            selectedSnapshotId={
              groupDetailSnapshots.length > 0
                ? currentGroupDetailSnapshots[currentGroupDetailIndex]
                    ?.snapshotId
                : selectedStockSnapshots.length > 0
                ? filteredStockSnapshots[currentStockSnapshotIndex]?.snapshotId
                : groupedSnapshots.length > 0
                ? groupedSnapshots[currentGroupedSnapshotIndex]
                    ?.memberStockSnapshotId
